...
Figure<!--s 2 and--> 3 provides an example of damage functions developed for a default building in the case of a flood with a duration d = 24 h, flow velocity v = 2.0 m/s, sediment concentration s = 0.05, and presence of pollutants (q = 1).
...
ComputeDamage <- function(he, v, d, s, q, 
  ...
  frag2_1f <- round(ptruncnorm(h, a = 0.2, b = 0.6, mean = 0.4, sd = 0.4/6), 3)
  frag2_2f <- round(ptruncnorm(h, a = 0.2 + IH, b = 0.6 + IH, mean = 0.4 + IH, sd = 0.4/6), 3)
  # Partitions: 1.5 - 2.0 m
  frag3_1f <- round(ptruncnorm(h, a = 1.5, b = 2.0, mean = 1.75, sd = 0.5/6), 3)
  frag3_2f <- round(ptruncnorm(h, a = 1.5 + IH, b = 2.0 + IH, mean = 1.75 + IH, sd = 0.5/6), 3)
  # External plaster and doors: 1.0 - 1.5 m/s
  frag4 <- round(ptruncnorm(v, a = 1, b = 1.5, mean = 1.25, sd = 0.5/6), 3)
  # Doors: 0.4 - 0.8 m
  frag5_1f <- round(ptruncnorm(h, a = 0.4, b = 0.8, mean = 0.6, sd = 0.4/6), 3)
  frag5_2f <- round(ptruncnorm(h, a = 0.4 + IH, b = 0.8 + IH, mean = 0.6 + IH, sd = 0.4/6), 3)
  # Windows: 1.2 - 1.8 m
  frag6_1f <- round(ptruncnorm(h, a = 1.2, b = 1.8, mean = 1.5, sd = 0.5/6), 3)
  frag6_2f <- round(ptruncnorm(h, a = 1.2 + IH, b = 1.8 + IH, mean = 1.5 + IH, sd = 0.5/6), 3)
  # Windows: 0.8 - 1.0 m/s
  frag7 <- round(ptruncnorm(v, a = 0.8, b = 1, mean = 0.9, sd = 0.2/6), 3)
  # Structural damage
  ...
  R5 <- up["plasterboardremoval",] *                               # unit price
          IA * 0.2 * pmin(nf, ceiling((h-(IH-0.5))/IH)) * (FL>1) *
          (1 - 0.2*(BT==3))                                        # economies of scale
...
if (!uncert) {
  ...
  for (i in 1:length(he)) {
    ...
    # to a matrix.
    statMat[i, 1] <- quantile(modelOutput$absDamage, 0.05)
    statMat[i, 2] <- mean(modelOutput$absDamage)
    statMat[i, 3] <- quantile(modelOutput$absDamage, 0.95)
