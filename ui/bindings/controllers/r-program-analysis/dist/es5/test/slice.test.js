...
describe('slice', function () {
    it('statements including the def and use', function () {
        var ast = python_parser_1.parse(['a = 1', 'b = a', ''].join('\n'));
        var locations = slice_1.slice(ast, new slice_1.LocationSet(loc(2, 0, 2, 5)));
        chai_1.expect(locations.items).to.deep.include(loc(1, 0, 1, 5));
    });
    it('at least yields the statement for a seed', function () {
        var ast = python_parser_1.parse(['c = 1', ''].join('\n'));
        var locations = slice_1.slice(ast, new slice_1.LocationSet(loc(1, 0, 1, 2)));
        chai_1.expect(locations.items).to.deep.include(loc(1, 0, 1, 5));
    });
    it('does our current demo', function () {
        ...
        var da = new data_flow_1.DataflowAnalyzer();
        var locations = slice_1.slice(ast, new slice_1.LocationSet(loc(8, 0, 8, 46)), da);
        var lineNums = locations.items.map(function (loc) { return loc.first_line; });
        [1, 2, 3, 4, 5, 7, 8].forEach(function (line) {
            return chai_1.expect(lineNums).to.deep.include(line);
        });
        chai_1.expect(lineNums).to.not.include(6);
    });
    it('uses api specs to decide mutating methods', function () {
        ...
        var da = new data_flow_1.DataflowAnalyzer();
        var criterion = new slice_1.LocationSet(loc(5, 0, 5, 12));
        var locations = slice_1.slice(ast, criterion, da);
        var lineNums = locations.items.map(function (loc) { return loc.first_line; });
        [1, 2, 3, 5].forEach(function (line) {
            return chai_1.expect(lineNums).to.include(line);
        });
        chai_1.expect(lineNums).to.not.include(4);
    });
    it('joins inferred types', function () {
        ...
        var criterion = new slice_1.LocationSet(loc(9, 0, 9, 12));
        var locations = slice_1.slice(ast, criterion, new data_flow_1.DataflowAnalyzer());
        var lineNums = locations.items.map(function (loc) { return loc.first_line; });
        [1, 2, 3, 4, 5, 6, 7, 9].forEach(function (line) {
            return chai_1.expect(lineNums).to.include(line);
        });
        chai_1.expect(lineNums).to.not.include(8);
    });
    it('does the documentation example', function () {
        ...
        var da = new data_flow_1.DataflowAnalyzer();
        var criterion = new slice_1.LocationSet(loc(6, 0, 6, 10));
        var locations = slice_1.slice(ast, criterion, da);
        var lineNums = locations.items.map(function (loc) { return loc.first_line; });
        ...
        [2, 5].forEach(function (line) {
            return chai_1.expect(lineNums).to.not.include(line);
        });
    });
    it('eliminates functions without side-effects', function () {
        ...
        var da = new data_flow_1.DataflowAnalyzer();
        var criterion = new slice_1.LocationSet(loc(6, 0, 6, 8));
        var locations = slice_1.slice(ast, criterion, da);
        var lineNums = locations.items.map(function (loc) { return loc.first_line; });
    ...
    it('keeps functions with field updates', function () {
        ...
        var da = new data_flow_1.DataflowAnalyzer();
        var criterion = new slice_1.LocationSet(loc(11, 0, 11, 8));
        var locations = slice_1.slice(ast, criterion, da);
        var lineNums = locations.items.map(function (loc) { return loc.first_line; });
    ...
    it("does jim's demo", function () {
        ...
        var criterion = new slice_1.LocationSet(loc(7, 0, 7, 21));
        var locations = slice_1.slice(ast, criterion, new data_flow_1.DataflowAnalyzer());
        var lineNums = locations.items.map(function (loc) { return loc.first_line; });
